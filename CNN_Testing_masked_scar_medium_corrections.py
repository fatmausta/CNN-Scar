# -*- coding: utf-8 -*-
"""
Created on Tue Oct  3 09:49:35 2017
...
#    y_predP = model.predict(X_testing, verbose = 2)    
...
for pid in pid_test:
    ...
    for sl in range(0, scarGT.shape[0]):
        if(numpy.sum(scarGT[sl,:,:]))>0:
            scar_depth[0,0] = sl
            break
        
    for sl in range(scarGT.shape[0]-1, 0, -1):
        if(numpy.sum(scarGT[sl,:,:]))>0:
            scar_depth[0,1] = sl
            break
    
    BW1 = numpy.empty(scarGT.shape)
    BW2 = numpy.empty(scarGT.shape)
    
    BW1 = scarGT[range(scar_depth[0,0], scar_depth[0,1]),:,:]
    BW2 = scarCNN[range(scar_depth[0,0], scar_depth[0,1]),:,:]
...
print(pid_test)
print(Dice_list)
print('\nmean Dice over %d testing images: %2.3f percent \n\n' %(len(pid_test),np.mean(Dice_list)))